...
login.bind_events = function () {
	// document.querySelector("#input-otp-login-div").style.display="none";
	const theotpdiv=document.querySelector(".input-otp-login-div");
	const thephnodiv=document.querySelector(".input-phone-login-div");
	const loginnum=document.querySelector("#login_phno");
	const theotpbox = document.querySelector("#verify-otp")
	var theloginnum;
	...
	$( document ).ready(function() {
		...
		theotpdiv.style.display="none";

		theloginnum = window.intlTelInput(loginnum, {
//   initialCountry: "in",
  strictMode: true,
  ...
  utilsScript: "https://cdn.jsdelivr.net/npm/intl-tel-input@23.1.0/build/js/utils.js", // just for formatting/placeholders etc
	...
	$(".send-otp-button").on("click", function (event) {
		...
		var countrycode = theloginnum.getSelectedCountryData()['dialCode'];
		var fullnumber = theloginnum.getNumber();
		var enterednumber=fullnumber.replace(("+"+countrycode),"");
		// console.log("uuuuuuuuuuuu          ",countrycode);
		// console.log("uuuuuuuuuuuu          ",fullnumber );
		// console.log("uuuuuuuuuuuu          ",enterednumber );
		var selected = theloginnum.getSelectedCountryData();
		// console.log("uuuuuuuuuuuu          ",enterednumber );
		console.log("oooooooooooooooooooooooooooo          ",selected );

		frappe.call({
			method: "gsc.api.generate_otp",
			args: {
				role_profile: 'Test',
				country_code :("+"+countrycode),
				phone_no : enterednumber,
				countryname:selected['name'],
				shortcountryname:selected['iso2'],
			},
			...
			success: (r)=> {
				// on success
				console.log("fffffffffffffffffffff   ",r);
				theotpdiv.style.display="inline";
				thephnodiv.style.display="none";
	...
	$(".verify-otp-button").on("click", function (event) {
		...
		var fullnumber = theloginnum.getNumber()
		var enterednumber=fullnumber.replace(("+"+countrycode),"")
		console.log("oooooooooooooooo         ",countrycode);
		console.log("oooooooooooooooo         ",fullnumber );
		console.log("oooooooooooooooo         ",enterednumber );

		var selected = theloginnum.getSelectedCountryData();


		frappe.call({
			method: "gsc.api.verify_otp",
			args: {
				role_profile: 'Test',
				country_code :("+"+countrycode),
				phone_no : enterednumber,
				verify_otp : theotpbox.value,
				countryname:selected['name'],
				shortcountryname:selected['iso2']
			},
			// disable the button until the request is completed
			...
			success: function(r) {
				...
				document.querySelector("#input-otp-login-div").style.display="inline";
				document.querySelector("#input-phone-login-div").style.display="none";
